...
Binary assets — IFC models, point cloud tiles, PDFs, images — are stored in **MinIO** (S3-compatible). Each File record in PostgreSQL holds the metadata (name, type, size, owner, parent building); the binary itself lives in MinIO and is referenced by an object key.
